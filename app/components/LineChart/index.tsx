...
const LineChart: FC<Props> = ({ selectedCoin, getLatestCoinPrice }) => {
  ...
  useEffect(() => {
    ...
    const getBatch = () => Array.from({ length: batchSize }, (_, i) => COMMENTS[(index + i) % COMMENTS.length]);
        ...
        {comments.map(({ name, date, action, color }, idx) => {
          ...
          return (
            <div
              key={name}
              className={cn(
                "absolute bottom-[40px] transition duration-500 not-last:mb-2",
                isLast && "translate-y-[110%]! text-red-500 opacity-0",
                isFirst && "-translate-y-[110%]! opacity-20",
              )}
            >
              <div className="flex items-center gap-1">
                <h4 style={{ color }} className="text-sm font-medium">
                  {name}
                </h4>
                <dl className="text-xs text-[#73767D]">
